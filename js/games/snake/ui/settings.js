 ...
 * settings store.
 *
 * Speed and wrap are rules, not preferences: they change what a score means.
 * The panel says so, and the core applies them only on the next start.
 */
...
export function createSettingsUI(overlays, applySettings) {
  ...
  function show() {
    ...
    for (const key of Object.keys(SPEEDS)) {
      const { label, multiplier } = SPEED_TABLE[key];
      speedSelect.appendChild(el('option', {
        text: `${label} (${multiplier}× points)`,
        attrs: { value: key }
      }));
    }
    speedSelect.value = currentSettings.speed;

    const wrapCheck = checkbox('set-wrap');
    wrapCheck.checked = currentSettings.wrap;
    ...
    form.appendChild(row('Wrap around walls', wrapCheck, 'Leave one edge, reappear at the other.'));
