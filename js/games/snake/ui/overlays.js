/**
 * Snake start, pause and game-over overlays.
 *
 * The card mechanism (focus handling, focus trap, activation guard) is shared
 * in js/shared/ui/overlays.js; this supplies the Snake copy and drives the
 * overlay from the Snake FSM.
 */
...
export function createOverlays({ onAction, getSettings = () => ({}) }) {
  ...
  function rulesLine() {
    ...
    const speed = SPEED_TABLE[settings.speed]?.label ?? 'Snake';
    const walls = settings.wrap ? 'Wrap: walls are open' : 'Walls are solid';
    return `${speed} speed   ·   ${walls}`;
  }
    ...
    showGameOver(state) {
      open('gameover', {
        ...
        body: `Score ${state.score.toLocaleString()}\n`
            + `Apples ${state.apples}   Length ${state.body.length}`,
