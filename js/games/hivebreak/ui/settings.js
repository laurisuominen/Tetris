 ...
 * settings store.
 *
 * Difficulty is a rule, not a preference: it changes what a score means. The
 * panel says so, and the core applies it only on the next start.
 */
...
export function createSettingsUI(overlays, applySettings) {
  ...
  function show() {
    ...
    const autoFireCheck = checkbox('set-autofire');
    autoFireCheck.checked = currentSettings.autoFire;
    ...
    form.appendChild(row('Difficulty', speedSelect, 'Applies to your next game.'));
    form.appendChild(row(
      'Drag to fly', pointerCheck,
      'Drag anywhere on the field and the ship moves with your finger. Off leaves the keys and arrows.'
    ));
    form.appendChild(row(
      'Auto-fire', autoFireCheck,
      'The gun fires by itself. It is never faster than holding the trigger — the cooldown is the same.'
    ));
    ...
    on(saveBtn, 'click', () => {
      currentSettings = {
        ...
        autoFire: autoFireCheck.checked,
