 ...
 * governed by that same directive, so inline styling here would be dropped by
 * the browser and the card would render unstyled.
 *
...
export function createScoresView(overlays) {
  ...
  function showGameOver(state) {
    ...
    // shortcut deliberately ignores this field — so without this, Enter would
    // do nothing at all.
  ...
  function showLeaderboardOnly(state, unlocked = []) {
    ...
    overlays.open(state ? 'gameover' : 'leaderboard', {
      title: state ? (state.won ? 'Maze Master' : 'Game Over') : 'High Scores',
      body: container,
      buttons
    });
  }
