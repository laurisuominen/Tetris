/**
 * Chomp start, pause and game-over overlays.
 *
 * The card mechanism (focus handling, focus trap, activation guard) is shared in
 * js/shared/ui/overlays.js; this supplies the Chomp copy and drives the overlay
 * from the Chomp FSM.
 */
...
export function createOverlays({ onAction, getSettings = () => ({}) }) {
  ...
  function rulesLine() {
    ...
    const ai = settings.modernAI ? 'modern ghosts' : 'arcade ghosts';
    return `${tier.label} — ${tier.blurb}\n${START_LIVES} lives   ·   ${ai}`;
  }
  ...
  return {
    ...
    showGameOver(state) {
      open('gameover', {
        title: state.won ? 'Maze Master' : 'Game Over',
