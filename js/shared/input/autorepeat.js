 ...
 * boolean.
 *
 * Timings are required arguments rather than imported constants, so each game
 * owns its own feel.
 */

export function createAutoRepeat({ das, arr } = {}) {
  if (!(das >= 0) || !(arr >= 0)) {
    throw new Error('createAutoRepeat requires das and arr in milliseconds');
  }
